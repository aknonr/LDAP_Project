...
/// <summary>
/// IIS WebApplication icin apppool identity hedef hesapla eslesiyorsa sifreyi gunceller.
/// </summary>
public sealed class IisWebAppUpdateStrategy : RemoteJsonUpdateStrategyBase
{
    public IisWebAppUpdateStrategy(
        IRemoteCommandExecutor remoteCommandExecutor,
        ILogger<IisWebAppUpdateStrategy> logger)
    ...
    protected override string BuildScript(Domain.Entities.JobResource resource, string targetAccount, string newPassword)
    {
        ...
        {
            return """
[PSCustomObject]@{ state = 'FAILED'; code = 'RESOURCE_NOT_FOUND'; message = 'IIS web app apppool bilgisi bulunamadi.' } | ConvertTo-Json -Compress
""";
        }
...
try {
    ...
    [PSCustomObject]@{ state = 'UPDATED'; code = 'OK'; message = 'IIS web app apppool credentials guncellendi.' } | ConvertTo-Json -Compress
}
catch {
