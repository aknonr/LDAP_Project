...
/// <summary>
/// Discovery result event'lerini DB + SignalR tarafina uygular.
/// </summary>
public sealed class ServerUsageResultEventConsumer : IConsumer<ServerUsageResultEvent>
{
    private readonly IJobTrackingService _jobTrackingService;
    private readonly IHubContext<JobsHub> _hubContext;
    private readonly ILogger<ServerUsageResultEventConsumer> _logger;

    public ServerUsageResultEventConsumer(
        IJobTrackingService jobTrackingService,
        IHubContext<JobsHub> hubContext,
        ILogger<ServerUsageResultEventConsumer> logger)
    {
    ...
    public async Task Consume(ConsumeContext<ServerUsageResultEvent> context)
    {
        ...
        {
            _logger.LogWarning(
                "ServerUsageResultEvent atlandi. Job/Target bulunamadi. JobId={JobId}, TargetId={TargetId}",
                message.JobId,
                message.TargetId);
            return;
        }
